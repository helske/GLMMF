#' Fit Generalized Linear Mixed-Effects Models with Latent Factors Using State Space Framework
#'
...
glmmf<-
  function(group,response,common.fixed,distinct.fixed,random,nfactors=0,data,
           ...
           tol=.Machine$double.eps^0.5,trace=0,gradient=FALSE,...){
    ...
    res<-fitGLMMF(model=model,estimate.dispersion=estimate.dispersion,common.dispersion=common.dispersion,
                  ...
                  init.factor=init.factor,init.theta=init.theta,estimate=estimate,trace=trace,gradient=gradient,...)    
    model<-res$model

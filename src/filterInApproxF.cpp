...
//  
//  Rcpp::Rcout<<"PtAll.slice(1) "<<PtAll.slice(1)<<std::endl;
//   Rcpp::Rcout<<"PtAll.slice(2) "<<PtAll.slice(2)<<std::endl;
//    Rcpp::Rcout<<"rt.row(2) "<<rt.row(2)<<std::endl;
//    Rcpp::Rcout<<"rt0.row(2) "<<rt0.row(2)<<std::endl;
//    Rcpp::Rcout<<"rt1.row(2) "<<rt1.row(2)<<std::endl;
//    Rcpp::Rcout<<"coefs.row(2) "<<coefs.row(2)<<std::endl;
//    Rcpp::Rcout<<"coefs.row(2) "<<coefs.row(2)<<std::endl;
//    Rcpp::Rcout<<"PtAll.slice(n-1).diag().t() "<<PtAll.slice(n-1).diag().t()<<std::endl;
//        Rcpp::Rcout<<"PtAll.slice(n-2).diag().t() "<<PtAll.slice(n-2).diag().t()<<std::endl;
...
//  
//  arma::mat diagm1 = arma::eye<arma::mat>(m1,m1);
//  
//
//  //Rcpp::Rcout<<ft<<std::endl;
//  //Rcpp::Rcout<<Z.slice((n-1)).col((p-1))*vt((p-1),(n-1))/ft((p-1),(n-1))<<std::endl;
//  // non-diffuse phase from n to d+1
//  for(int t=(n-1);t>d;t--){
//    for(int i=(p-1);i>=0;i--){
//      ind=t;
//      //tmpm = kt.slice(t).col(i); 
//      rt(zind.col(i),ind) += Z.slice(t).col(i)*vt(i,t)/ft(i,t)-
//      Z.slice(t).col(i)*kt.slice(t).col(i).t()/ft(i,t)*rt.col(t);
//      //rt(zind.col(i),ind+1) -= Z.slice(t).col(i)*kt.slice(t).col(i).t()/ft(i,t)*rt.col(t+1);
//      
//    }
//    
//    
...
//  }
//  if(d>=0){
//    // non-diffuse phase at time d
//    ind=d;
//    for(int i=(p-1);i>j;i--){    
//      //tmpm = kt.slice(d).col(i);
//      rt(zind.col(i),ind) += Z.slice(d).col(i)*vt(i,d)/ft(i,d)-
//      Z.slice(d).col(i)*kt.slice(d).col(i).t()/ft(i,d)*rt.col(d);
//      //rt(zind.col(i),ind+1) = Z.slice(d).col(i)*vt(i,d)/ft(i,d) + 
//      //(diagm1 - Z.slice(d).col(i)*tmpm.rows(zind.col(i)).t()/ft(i,d))*rt(zind.col(i),ind+1);
//      
//    }
//    // diffuse phase at time d
//    for(int i=j;i>=0;i--){    
//      if(finf(i,d)>tol){
//        rt(zind.col(i),ind) -= Z.slice(d).col(i)*kinf.slice(d).col(i).t()/finf(i,d)*rt.col(d);
//        //tmpm = kinf.slice(d).col(i);
//        //rt(zind.col(i),ind+1) = (diagm1 - Z.slice(d).col(i)*tmpm.rows(zind.col(i)).t()/finf(i,d))*rt(zind.col(i),ind+1);
//      } else {
//        rt(zind.col(i),ind) += Z.slice(d).col(i)*vt(i,d)/ft(i,d)-
//        Z.slice(d).col(i)*kt.slice(d).col(i).t()/ft(i,d)*rt.col(d);
//        //tmpm = kt.slice(d).col(i);
...
//      }
//    }
//    // diffuse phase from d-1 to 1
//    if(nfactors<m && d>0){
//      rt.submat(nfactors,d-1,m-1,d-1)=rt.submat(nfactors,d,m-1,d);
//    }
//    for(int t=(d-1);t>=0;t--){
//      ind=t;
//      for(int i=(p-1);i>=0;i--){
//        if(finf(i,t)>tol){
//          rt(zind.col(i),ind) -= Z.slice(t).col(i)*kinf.slice(t).col(i).t()/finf(i,t)*rt.col(t);
//          //    tmpm = kinf.slice(t).col(i);
//          //    rt(zind.col(i),ind+1) = (diagm1 - Z.slice(t).col(i)*tmpm.rows(zind.col(i)).t()/finf(i,t))*rt(zind.col(i),ind+1);
//        } else {
//          rt(zind.col(i),ind) += Z.slice(t).col(i)*vt(i,t)/ft(i,t)-
//          Z.slice(t).col(i)*kt.slice(t).col(i).t()/ft(i,t)*rt.col(t);   // TARKISTA!!
//          //tmpm = kt.slice(t).col(i);
//          //rt(zind.col(i),ind+1) = Z.slice(t).col(i)*vt(i,t)/ft(i,t) + 
//          //(diagm1 - Z.slice(t).col(i)*tmpm.rows(zind.col(i)).t()/ft(i,t))*rt(zind.col(i),ind+1);
//          
//        }
//        if(nfactors<m && t>0){
//          rt.submat(nfactors,t-1,m-1,t-1)=rt.submat(nfactors,t,m-1,t);
